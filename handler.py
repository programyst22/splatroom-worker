...
def handler(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if not str(data["output_path"]).lower().endswith(".sog"):
        raise WorkerError("output_path must end with .sog")
    ...
    try:
        ...
        report(job, data, "exported", 91, metrics=metrics)

        # Standard Gaussian PLYs are large and can exceed Supabase Free's
        # per-object upload limit. Convert to PlayCanvas SOG: substantially
        # smaller and the preferred format for browser/mobile runtime use.
        report(job, data, "compressing", 92)
        t0 = time.time()
        sog = export_dir / "scene.sog"
        run_command(
            [
                "splat-transform",
                "--no-tty",
        ...
        if not sog.exists() or sog.stat().st_size <= 0:
            raise WorkerError("SOG conversion finished but scene.sog was not produced")
        metrics["compress_seconds"] = round(time.time() - t0, 2)
        metrics["output_bytes"] = sog.stat().st_size
        metrics["compression_ratio"] = round(
            metrics["ply_bytes"] / max(1, metrics["output_bytes"]), 2
        )
        report(job, data, "compressed", 94, metrics=metrics)

        # Supabase signed upload URLs are short-lived, so request one only now.
        upload_url = get_fresh_upload_url(data)
        report(job, data, "upload", 96)
        t0 = time.time()
        upload_file(upload_url, sog)
