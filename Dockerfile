...
# PlayCanvas SplatTransform converts the large Nerfstudio PLY to SOG,
# the compact runtime format recommended for web/mobile delivery.
RUN curl -fsSL https://deb.nodesource.com/setup_22.x | bash - \
    && apt-get update \
    && apt-get install -y --no-install-recommends nodejs \
    && npm install -g @playcanvas/splat-transform@3 \
